...
contract MerklGaugeMiddleman {
    ...
    constructor(ICore _core) {
        ...
        if (address(_angle) != address(0))
            _angle.safeIncreaseAllowance(address(merklRewardManager()), type(uint256).max);
    }
    ...
    function merklRewardManager() public view virtual returns (DistributionCreator) {
    ...
    function setAngleAllowance() external {
        IERC20 _angle = angle();
        address manager = address(merklRewardManager());
    ...
    function setGauge(address gauge, DistributionParameters memory params) external {
        ...
        DistributionCreator manager = merklRewardManager();
        if (
            gauge == address(0) ||
    ...
    function notifyReward(address gauge, uint256 amount) external {
        ...
        if (amount > 0) merklRewardManager().createDistribution(params);
    }
}
