...
// Multi-chain upgrade tests - run with: forge test --match-contract UpgradeDistributorMultiChainTest
contract UpgradeDistributorMultiChainTest is UpgradeDistributorTest {
    struct ChainConfig {
        uint256 chainId;
        string rpcEnvVar;
        string name;
    ...
    function test_AllChains_VerifyStorageSlots() public {
        ChainConfig[] memory chains = getChainConfigs();
        
        for (uint256 i = 0; i < chains.length; i++) {
            ChainConfig memory config = chains[i];
            
            // Skip if RPC URL not set
            try vm.envString(config.rpcEnvVar) returns (string memory rpcUrl) {
                if (bytes(rpcUrl).length == 0) continue;
                
                console.log("Testing chain:", config.name);
                
                // Create fork for this chain
                vm.createSelectFork(rpcUrl);
                chainId = block.chainid;
                require(chainId == config.chainId, "Chain ID mismatch");
                
                // Load distributor and setup
                distributor = Distributor(0x3Ef3D8bA38EBe18DB133cEc108f4D14CE00Dd9Ae);
                _setupUpgradeTest();
                
                // Run storage verification
                test_VerifyStorageSlots_Success();
                
                console.log("Chain", config.name, "passed!");
            } catch {
                console.log("Skipping chain", config.name, "- RPC not configured");
            }
        }
    }

    function test_AllChains_UpdateTree() public {
        ChainConfig[] memory chains = getChainConfigs();
        ...
        for (uint256 i = 0; i < chains.length; i++) {
            ...
            try vm.envString(config.rpcEnvVar) returns (string memory rpcUrl) {
                ...
                console.log("Testing UpdateTree on chain:", config.name);
                ...
                // Run update tree test
                test_UpdateTree_Success_WhenAuthorizedUpdater();
                
                console.log("Chain", config.name, "UpdateTree passed!");
            } catch {
                console.log("Skipping chain", config.name, "- RPC not configured");
            }
        }
