...
for chain in "${CHAINS[@]}"; do
  echo "=== $chain ==="
  if forge script scripts/setMessageMultichain.s.sol:SetMessageMultichain --rpc-url "$chain" --sender 0xA9DdD91249DFdd450E81E1c56Ab60E1A62651701 --broadcast; then
    SUCCESS+=("$chain")
  else
    echo "FAILED: $chain"
    FAILED+=("$chain")
